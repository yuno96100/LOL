    ...
    try {
        ...
        if (session.type === "DIRECT") {
            ...
            if (!session.data) { 
                ...
                if (session.waitAction === "가입_PW") { Database.data[session.tempId] = {pw:msg, gold:1000, level:1}; Database.save(Database.data); session.waitAction = null; return replier.reply("가입 성공!"); }
                ...
                if (msg === "3") { session.waitAction = "문의_내용작성"; return replier.reply("문의내용 입력:"); }
            } else { 
                if (msg === "3") { session.data = null; return replier.reply(UI.make("로그아웃", "완료")); }
                if (msg === "4") { session.waitAction = "문의_내용작성"; return replier.reply("문의내용 입력:"); }
