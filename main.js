...
var UserManager = {
    handle: function(msg, session, replier) {
        ...
        // 2. 로그인 상태 로직 (USER_MAIN)
        if (session.screen === "USER_MAIN" || session.screen === "IDLE") {
            if (session.screen === "IDLE") session.screen = "USER_MAIN";
        ...
        // --- 나머지 하위 메뉴 로직 (이전과 동일하므로 생략 없이 유지) ---
...
var SessionManager = {
    ...
    reset: function(session) { 
        ...
        session.battle = null;
        // 데이터가 없는 경우 아이디를 비회원으로 강제 고정
        if (!session.data) session.tempId = "비회원";
    },
    forceLogout: function(userId) {
        for (var key in this.sessions) { 
            if (this.sessions[key].tempId === userId) { 
                ...
                this.sessions[key].type = "DIRECT"; // 권한 초기화
                this.sessions[key].screen = "IDLE"; // 화면 상태 초기화
...
function response(room, msg, sender, isGroupChat, replier, imageDB) {
    ...
    try {
        ...
        // 2. '메뉴' 입력 시 처리
        if (msg === "메뉴") {
            // 로그인 데이터가 없거나 이미 IDLE 상태면 즉시 초기 메뉴 출력
            if (!session.data || session.tempId === "비회원" || session.screen === "IDLE") {
                SessionManager.reset(session);
                return replier.reply(UI.renderMenu(session));
            }
            // 그 외(진행 중인 작업 있음)에는 중단 확인창 띄움
            return showCancelConfirm(session, replier);
        }

        // 3. 배틀 드래프트 등 특수 화면 우선 처리
        if (session.screen && session.screen.indexOf("BATTLE_DRAFT") !== -1) {
            return MatchingManager.handleDraft(msg, session, replier);
        } else {
            return handleGeneralMenu(msg, session, sender, replier);
