/**
 * [main.js] v0.0.07_UI_Fix
 * 1. UI 문구 수정: 안내문은 격식체를 유지하되, 카테고리 항목은 간결하게 복구
 * 2. LoginManager 로직: LOGIN_ID -> LOGIN_PW 연결 오류 수정 완료
 */

// ━━━━━━━━ [1. 설정 및 상수] ━━━━━━━━
var Config = {
    Version: "v0.0.07_UI_Fix",
    Prefix: ".",
    AdminRoom: "소환사의협곡관리", 
    BotName: "소환사의 협곡",
...
var UI = {
    ...
    renderMenu: function(session) {
        ...
        if (!session.data) return this.go(session, "GUEST_MAIN", "환영합니다", "1. 새로운 회원가입\n2. 기존 계정 로그인\n3. 운영진에게 문의하기", "메뉴 선택");
        return this.go(session, "USER_MAIN", "메인 로비", "1. 개인 프로필 조회\n2. 컬렉션 확인\n3. 대전 모드 진입\n4. 상점 이용\n5. 운영진 문의하기\n6. 안전하게 로그아웃", "메뉴 선택");
    }
};
...
var AdminActions = {
    showSysInfo: function(session, replier) {
        ...
        replier.reply(UI.go(session, "ADMIN_SYS_INFO", "시스템 상태 정보", info, "조회 완료"));
    },
    showUserList: function(session, replier) {
        ...
        var list = session.userListCache.map(function(id, i){ 
            ...
            var badge = (qCount > 0) ? " [🔔 문의: " + qCount + "]" : "";
            return (i+1) + ". " + id + badge; 
        }).join("\n");
        replier.reply(UI.go(session, "ADMIN_USER_LIST", "전체 유저 목록", list, "유저 번호 입력"));
    },
    submitAnswer: function(msg, session, replier) {
        var targetRoom = SessionManager.findUserRoom(session.targetUser);
        Api.replyRoom(targetRoom, UI.make("운영진 회신", "보내신 문의 사항에 대한 운영진의 답변입니다.\n\n[답변 내용]\n" + msg, "소환사의 협곡 운영진 드림", true));
        if(Database.data[session.targetUser]) { Database.data[session.targetUser].inquiryCount = 0; Database.save(Database.data); }
        SessionManager.reset(session);
        replier.reply(UI.go(session, "SUCCESS_IDLE", "전송 완료", "답변이 정상적으로 전달되었습니다.", "관리자 메뉴 복귀"));
    },
    editUserData: function(msg, session, replier) {
        var val = parseInt(msg);
        if (isNaN(val)) return replier.reply(UI.make("오류", "수치 수정을 위해 숫자만 입력하십시오.", "다시 입력"));
        Database.data[session.targetUser][session.editType] = val;
        Database.save(Database.data);
        Api.replyRoom(SessionManager.findUserRoom(session.targetUser), UI.make("알림", "운영진에 의해 계정의 [" + (session.editType === "gold" ? "골드" : "LP") + "] 정보가 조정되었습니다.", "운영 정책 조치", true));
        SessionManager.reset(session);
        replier.reply(UI.go(session, "SUCCESS_IDLE", "수정 완료", "데이터가 정상적으로 반영되었습니다.", "관리자 메뉴 복귀"));
    },
    resetConfirm: function(msg, session, replier) {
        if (msg === "확인") {
            ...
            SessionManager.reset(session);
            replier.reply(UI.go(session, "SUCCESS_IDLE", "초기화 완료", "데이터 초기화가 완료되었습니다.", "관리자 메뉴 복귀"));
        }
    },
    deleteConfirm: function(msg, session, replier) {
        if (msg === "삭제확인") {
            Api.replyRoom(SessionManager.findUserRoom(session.targetUser), UI.make("알림", "운영진에 의해 계정이 삭제 처리되었습니다.", "관리자 조치", true));
            delete Database.data[session.targetUser];
            Database.save(Database.data);
            SessionManager.forceLogout(session.targetUser);
            SessionManager.reset(session);
            replier.reply(UI.go(session, "SUCCESS_IDLE", "삭제 완료", "해당 계정이 완전히 삭제되었습니다.", "관리자 메뉴 복귀"));
        }
    }
};
...
var UserActions = {
    handleInquiry: function(msg, session, replier) {
        if (session.data) {
            ...
            Database.save(Database.data);
            Api.replyRoom(Config.AdminRoom, UI.make("유저 문의", "계정명: " + session.tempId + "\n내용: " + msg, "신속 답변 요망", true));
        } else {
            Api.replyRoom(Config.AdminRoom, UI.make("비회원 문의", "발신: " + session.room + "\n내용: " + msg, "회신 불가", true));
        }
        SessionManager.reset(session);
        replier.reply(UI.make("접수 완료", "문의하신 내용이 정상 접수되었습니다.\n검토 후 회신드리겠습니다.", "이용 감사드립니다.", true));
    },
    showCollection: function(msg, session, replier) {
        var d = session.data;
        if (session.screen === "COL_MAIN") {
            if (msg === "1") {
                var tList = d.collection.titles.map(function(t, i) { return (i+1) + ". " + (t === d.title ? "✅ [장착] " : "") + t; }).join("\n");
                return replier.reply(UI.go(session, "COL_TITLE_ACTION", "보유 칭호", tList, "장착 번호 입력"));
            }
            if (msg === "2") {
                var cList = (d.collection.champions.length > 0) ? d.collection.champions.join("\n") : "보유 챔피언 없음";
                return replier.reply(UI.go(session, "COL_CHAR_VIEW", "나의 챔피언", cList, "수집 목록"));
            }
        }
        if (session.screen === "COL_TITLE_ACTION") {
            ...
            if (d.collection.titles[idx]) {
                ...
                Database.save(Database.data);
                SessionManager.reset(session);
                return replier.reply(UI.make("설정 완료", "[" + d.title + "] 칭호를 장착하였습니다.", "프로필 반영 완료", true));
            }
        }
    },
    handleShop: function(msg, session, replier) {
        ...
        if (session.screen === "SHOP_MAIN" && msg === "1") {
            return replier.reply(UI.go(session, "SHOP_ROLES", "카테고리 선택", RoleKeys.map(function(r, i){ return (i+1)+". "+r; }).join("\n"), "번호 선택"));
        }
        if (session.screen === "SHOP_ROLES") {
            ...
            if (RoleKeys[rIdx]) {
                ...
                return replier.reply(UI.go(session, "SHOP_BUY_ACTION", session.selectedRole + " 상점", uList, "구매 번호 입력"));
            }
        }
        if (session.screen === "SHOP_BUY_ACTION") {
            ...
            if (units[uIdx]) {
                ...
                if (d.collection.champions.indexOf(target) !== -1) return replier.reply(UI.make("구매 불가", "이미 보유 중인 챔피언입니다.", "다른 대상 선택"));
                if (d.gold < 500) return replier.reply(UI.make("골드 부족", "보유 골드가 부족합니다.", "현재: " + d.gold + "G"));
    ...
    handleBattle: function(msg, session, replier) {
        if (session.screen === "BATTLE_MAIN" && msg === "1") {
            return replier.reply(UI.go(session, "BATTLE_AI_SEARCH", "매칭 중", "🤖 AI 대전 상대를 검색 중입니다...", "대기 중"));
        }
        ...
        if (session.screen === "BATTLE_PREP" && msg === "시작") {
            SessionManager.reset(session);
            return replier.reply(UI.make("알림", "대전 시스템 점검 중입니다. (v10 예정)", "메인 복귀", true));
        }
    }
};
    ...
    handle: function(msg, session, replier) {
        switch(session.screen) {
            ...
            case "ADMIN_USER_DETAIL":
                if (msg === "1") return replier.reply(UI.go(session, "ADMIN_EDIT_MENU", "데이터 수정", "1. 골드 수정\n2. LP 수정", "항목 선택"));
                if (msg === "2") return replier.reply(UI.go(session, "ADMIN_ANSWER_INPUT", "답변 작성", "["+session.targetUser+"] 내용 입력", "내용 작성"));
            ...
            case "ADMIN_EDIT_MENU":
                if (msg === "1") { session.editType = "gold"; return replier.reply(UI.go(session, "ADMIN_EDIT_INPUT", "골드 수정", "수치 입력", "수정 중")); }
                if (msg === "2") { session.editType = "lp"; return replier.reply(UI.go(session, "ADMIN_EDIT_INPUT", "LP 수정", "수치 입력", "수정 중")); }
                break;
...
var LoginManager = {
    handle: function(msg, session, replier) {
        switch(session.screen) {
            case "GUEST_MAIN":
                if (msg === "1") return replier.reply(UI.go(session, "JOIN_ID", "회원가입", "아이디 입력 (최대 10자)", "아이디 입력"));
                if (msg === "2") return replier.reply(UI.go(session, "LOGIN_ID", "인증", "아이디 입력", "아이디 입력"));
                if (msg === "3") return replier.reply(UI.go(session, "GUEST_INQUIRY", "비회원 문의", "내용 입력", "내용 입력"));
                break;
            case "JOIN_ID":
                if (msg.length > 10) return replier.reply(UI.make("오류", "최대 10자까지만 가능합니다.", "재입력"));
                if (Database.data[msg]) return replier.reply(UI.make("오류", "중복된 아이디입니다.", "아이디 변경"));
                session.tempId = msg;
                return replier.reply(UI.go(session, "JOIN_PW", "회원가입", "비밀번호 설정", "비밀번호 입력"));
            case "JOIN_PW":
                Database.data[session.tempId] = Database.getInitData(msg);
                Database.save(Database.data);
                session.data = Database.data[session.tempId];
                replier.reply(UI.make("성공", session.tempId + "님, 가입을 축하드립니다!", "자동 로그인 완료", true));
                SessionManager.reset(session);
                return replier.reply(UI.renderMenu(session));
            case "LOGIN_ID":
                session.tempId = msg;
                return replier.reply(UI.go(session, "LOGIN_PW", "본인 확인", "비밀번호 입력", "비밀번호 입력"));
            case "LOGIN_PW":
                if (Database.data[session.tempId] && Database.data[session.tempId].pw === msg) {
                    session.data = Database.data[session.tempId];
                    replier.reply(UI.make("성공", session.tempId + "님, 로그인되었습니다.", "환영합니다.", true));
                    SessionManager.reset(session);
                    return replier.reply(UI.renderMenu(session));
                }
                return replier.reply(UI.make("실패", "정보가 일치하지 않습니다.", "다시 시도"));
...
var UserManager = {
    handle: function(msg, session, replier) {
        switch(session.screen) {
            case "USER_MAIN":
                if (msg === "1") return replier.reply(UI.go(session, "PROFILE_VIEW", session.tempId, "", "정보 확인"));
                if (msg === "2") {
                    session.screen = "COL_MAIN";
                    return replier.reply(UI.go(session, "COL_MAIN", "컬렉션", "1. 보유 칭호 목록\n2. 보유 챔피언 목록", "항목 선택"));
                }
                if (msg === "3") {
                    session.screen = "BATTLE_MAIN";
                    return replier.reply(UI.go(session, "BATTLE_MAIN", "대전 모드", "1. AI 대전 시작", "번호 선택"));
                }
                if (msg === "4") {
                    session.screen = "SHOP_MAIN";
                    return replier.reply(UI.go(session, "SHOP_MAIN", "상점", "1. 챔피언 영입하기", "번호 선택"));
                }
                if (msg === "5") return replier.reply(UI.go(session, "USER_INQUIRY", "문의 접수", "내용을 입력하십시오.", "내용 입력"));
                if (msg === "6") { SessionManager.forceLogout(session.tempId); return replier.reply(UI.make("알림", "로그아웃되었습니다.", "이용 감사드립니다.", true)); }
                break;
...
function response(room, msg, sender, isGroupChat, replier, imageDB) {
    try {
        ...
        if (msg === "이전" && session.history && session.history.length > 0) {
            ...
            // UI를 다시 그리기 위해 렌더링 호출 (메뉴 형태 유지)
            if (session.screen.indexOf("MAIN") !== -1) return replier.reply(UI.renderMenu(session));
            return replier.reply(UI.go(session, session.screen, session.lastTitle, "", "이전 단계"));
        }
        ...
        Api.replyRoom(Config.AdminRoom, "🚨 오류: " + e.message + "\n라인: " + e.lineNumber); 
    }
}
