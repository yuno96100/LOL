/**
 * [main.js] v14.2.0
 * 1. 무생략 원칙: v10.0.0의 관리자(유저삭제/데이터초기화), 유저(문의/상점/컬렉션) 기능 전체 복구
 * 2. 단체방 확장: '2. 티어 랭킹' 기능 추가 (LP 기준 TOP 10 정렬)
 * 3. 로직 보존: 취소(예/아니오) 확인 및 이전 페이지 히스토리 버그 수정본 유지
 * 4. 보안: ".업데이트" 무시 및 관리자 해시 체크 유지
 */
...
var Config = {
    ...
    AdminRoom: "소환사의협곡관리",/**
 * [main.js] v14.2.1
 * - FIX: "Unterminated string literal" 유니코드 특수 공백 제거 및 문자열 정규화
 * - FIX: 개인톡방(DIRECT) 메뉴 로직 보정
 * - FEATURE: v14.2.0의 모든 관리자/유저 기능 유지
 */
...
var SessionManager = {
    ...
    get: function(r, h, g) {
        ...
        else if (g === true) s.type = "GROUP"; // 그룹채팅 여부 판정 보강
    ...
    handle: function(msg, session, replier) {
        ...
        if (session.screen === "SHOP_BUY_ACTION") {
            ...
            if (units[uIdx]) {
                ...
                d.gold -= 500; d.collection.characters.push(unitName); Database.save(Database.data);
...
var GroupManager = {
    handle: function(msg, session, replier) {
        var screen = session.screen;
        if (screen === "GROUP_MAIN") {
            if (msg === "1") return replier.reply(UI.go(session, "GROUP_PROFILE", session.tempId, "", "전적 확인"));
            if (msg === "2") {
                var users = Object.keys(Database.data);
                if (users.length === 0) return replier.reply(UI.make("알림", "등록된 유저가 없습니다."));
                var ranking = users.map(function(id) { return { id: id, lp: Database.data[id].lp || 0 }; }).sort(function(a, b) { return b.lp - a.lp; });
                var rankText = "", topCount = Math.min(ranking.length, 10);
                for (var i = 0; i < topCount; i++) {
                    var u = ranking[i], tier = getTierInfo(u.lp), medal = (i === 0) ? "🥇" : (i === 1) ? "🥈" : (i === 2) ? "🥉" : (i + 1) + ".";
                    rankText += medal + " " + u.id + "\n    └ " + tier.icon + " " + u.lp + " LP\n";
                }
                return replier.reply(UI.go(session, "GROUP_RANKING", "티어 랭킹 (TOP 10)", rankText, "전체 순위입니다."));
            }
        }
    }
};
...
function response(room, msg, sender, isGroupChat, replier, imageDB) {
    try {
        ...
        if (msg === "취소") {
            if (session.screen === "IDLE") return replier.reply(UI.make("알림", "진행 중인 작업 없음", "대기 중", true));
            ...
            return replier.reply(UI.go(session, "CANCEL_CONFIRM", "취소 확인", "정말로 작업을 취소하시겠습니까?\n\n'예' 입력 시 초기화됩니다.", "'예' 또는 '아니오'", true));
        }

        if (session.screen === "CANCEL_CONFIRM") {
            if (msg === "예" || msg === "1") { SessionManager.reset(session); return replier.reply(UI.make("알림", "작업이 취소되었습니다.", "'메뉴' 입력", true)); }
                ...
                if (s.indexOf("PROFILE") !== -1) return replier.reply(UI.renderProfile(session.tempId, session.data, h, c, isRoot));
        ...
        if (msg === "이전" && session.history && session.history.length > 0) {
            ...
            if (p.screen.indexOf("PROFILE") !== -1) return replier.reply(UI.renderProfile(session.tempId, session.data, p.help, p.content, isRoot));
        ...
        if (session.type === "ADMIN") {
            if (hash === Config.AdminHash) return AdminManager.handle(msg, session, replier);
            else return replier.reply("권한이 없습니다.");
        }
        ...
        if(Config.AdminRoom) Api.replyRoom(Config.AdminRoom, "오류: " + e.message + " (Line: " + e.lineNumber + ")"); 
    }
}/**
 * [main.js] v14.2.1
 * - FIX: "Unterminated string literal" 유니코드 특수 공백 제거 및 문자열 정규화
