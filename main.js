/**
 * [main.js] v7.7.1
 * 1. 계정명 복구: 세션의 ID 유지 로직을 강화하여 "알 수 없음" 현상 원천 차단.
 * 2. 구분선 정밀 교정: 최소 길이를 네비게이션 바 끝에 맞추고(18칸), 최대 길이는 채팅창 줄바꿈 방지(21칸).
 * 3. UI 동기화: 프로필 내부의 모든 구분선을 상하단 메인 구분선과 동일 규격으로 일치.
 * 4. 네비게이션: 간격을 1칸으로 조정하여 정갈한 레이아웃 완성.
 */
...
var Config = {
    ...
    // 모바일 채팅창 너비를 고려한 정밀 수치 (21칸 초과 시 줄바꿈 발생 위험)
    LIMITS: { MOBILE: 21, PC: 45 },
    // 네비게이션 바 끝에 맞춘 최소 구분선 길이
    MIN_LINE_LEN: 18 
};
...
var Utils = {
    ...
    getLineData: function(content, isPc) {
        ...
        // 최소 18칸(빨간선), 최대 21칸(노란선 이내) 사이에서 콘텐츠에 맞춰 유동적 결정
...
var UI = {
    ...
    renderProfile: function(id, data, isPc) {
        ...
        var accountId = id || "사용자"; // 계정명 유실 방지
            ...
            switch(session.screen) {
                ...
                case "LOGIN_PW":
                    if (Database.data[session.tempId] && Database.data[session.tempId].pw === msg) {
                        ...
                        // 로그인 성공 즉시 세션에 tempId 고정 (null 방지)
                        session.tempId = session.tempId;
...
function response(room, msg, sender, isGroupChat, replier, imageDB) {
    ...
    try {
        ...
        Api.replyRoom(Config.AdminRoom, "⚠️ [v7.7.1 에러]: " + e.message + " (L:" + e.lineNumber + ")");
    }
}var SystemData = {
    roles: {
        "탱커": { icon: "🛡️", units: ["알리스타", "말파이트", "레오나"] },
