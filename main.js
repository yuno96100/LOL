/**
 * [main.js] v0.0.09_AutoWrap_Full
 * 1. 자동 줄바꿈 엔진: Utils.wrapText (18자 한도) 적용
 * 2. 레이아웃 고정: 구분선 14개 (FIXED_LINE)
 * 3. 독립 모듈: Login, Admin, User 로직 누락 없이 전체 포함
 */
...
var Utils = {
    ...
    // 자동 줄바꿈 엔진: 모든 텍스트(content, help)에 자동 적용됨
...
function response(room, msg, sender, isGroupChat, replier, imageDB) {
    try {
        ...
        if (msg === "이전" && session.history && session.history.length > 0) {
            ...
            return replier.reply(UI.go(session, session.screen, session.lastTitle, "", "이전으로 돌아왔습니다."));
        }
