...
var AdminManager = {
    handle: function(msg, session, replier) {
        ...
        if (screen === "ADMIN_ANSWER_INPUT") {
            Api.replyRoom(session.targetUser, UI.make("운영진 답변", msg, "알림", true));
...
var UserManager = {
    handle: function(msg, session, replier) {
        // [세션 동기화] 현재 로그인된 상태라면 항상 데이터를 세션에 연결
        ...
        if (!d) {
            ...
            if (session.screen === "JOIN_PW") {
                ...
                // 핵심: 가입 후 바로 세션에 데이터 주입하여 로그인 상태로 전환
                session.data = Database.data[session.tempId]; 
            ...
            if (session.screen === "LOGIN_PW") {
                if (Database.data[session.tempId] && Database.data[session.tempId].pw === msg) {
                    session.data = Database.data[session.tempId]; // 세션 연결
                    SessionManager.reset(session);
                    replier.reply(UI.make("성공", "반갑습니다, " + session.tempId + "님!", "로그인 성공", true));
                    return replier.reply(UI.renderMenu(session));
                }
                return replier.reply(UI.make("실패", "인증 정보가 일치하지 않습니다.", "재시도"));
            }
            return;
        }
