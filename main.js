...
/**
 * [6. 관리자 매니저 - 완결본]
 * 시스템 정보 조회, 유저 데이터 제어, 문의 답변, 복구 기능을 포함합니다.
 */
var AdminManager = {
    handle: function(msg, session, replier) {
        ...
        if (session.screen === "ADMIN_MAIN") {
            if (msg === "1") return replier.reply(UI.go(session, "ADMIN_INFO", "시스템 정보", this.getSystemInfo()));
            if (msg === "2") return replier.reply(UI.go(session, "ADMIN_USER_LIST", "유저 관리", this.getUserList()));
            if (msg === "3") {
                var content = "⚠️ [주의] 현재 데이터를 백업본(.bak)으로 덮어씌웁니다.\n\n1. 즉시 복구\n2. 취소";
                return replier.reply(UI.go(session, "ADMIN_RESTORE", "데이터 복구", content, "1번 입력 시 실행"));
            }
        }

        // 2. 유저 상세 관리 (수정/답변/초기화/삭제)
        if (session.screen === "ADMIN_USER_DETAIL") {
            var target = session.targetUser;
            if (msg === "1") return replier.reply(UI.go(session, "ADMIN_EDIT_GOLD", "골드 수정", target + "님에게 지급할 골드량을 입력하세요."));
            if (msg === "2") return replier.reply(UI.go(session, "ADMIN_ANSWER", "문의 답변", target + "님에게 보낼 메시지를 입력하세요."));
            if (msg === "3") {
                Database.data[target] = Database.getInitData(Database.data[target].pw);
                Database.save(Database.data);
                return replier.reply(UI.make("알림", target + "님의 데이터를 초기화했습니다.", "메뉴로 돌아가기", true));
            }
            if (msg === "4") {
                delete Database.data[target];
                Database.save(Database.data);
                SessionManager.forceLogout(target);
                return replier.reply(UI.make("알림", target + " 계정을 삭제했습니다.", "메뉴로 돌아가기", true));
            }
        }

        // 3. 골드 수정 실행
        if (session.screen === "ADMIN_EDIT_GOLD") {
            var amt = parseInt(msg);
            if (isNaN(amt)) return replier.reply("⚠️ 숫자만 입력해주세요.");
            Database.data[session.targetUser].gold += amt;
            Database.save(Database.data);
            return replier.reply(UI.make("완료", session.targetUser + "님에게 " + amt + "G 지급 완료", "메뉴 입력", true));
        }

        // 4. 문의 답변 전송
        if (session.screen === "ADMIN_ANSWER") {
            var target = session.targetUser;
            // 해당 유저가 접속 중인 세션이 있다면 알림 (없어도 데이터베이스 방식 등으로 확장 가능)
            replier.reply(UI.make("답변 전송", target + "님께 답변을 보냈습니다.", "완료", true));
            // 실제 유저에게 메시지 발송 로직 (Api.replyRoom 활용 권장)
            return;
        }

        // 5. 데이터 복구 실행
        if (session.screen === "ADMIN_RESTORE" && msg === "1") {
            if (Database.restore()) {
                SessionManager.reset(session);
                return replier.reply("✅ 백업 데이터 복구 성공. 시스템을 재시작합니다.");
            }
            return replier.reply("❌ 복구 실패: 백업 파일이 없습니다.");
        }

        // 6. 유저 리스트에서 ID 입력 시 상세페이지 진입
        if (session.screen === "ADMIN_USER_LIST" && Database.data[msg]) {
            session.targetUser = msg;
            return replier.reply(UI.go(session, "ADMIN_USER_DETAIL", "유저 상세정보", "ID: " + msg, "관리 번호 선택"));
        }
    },
    ...
    getSystemInfo: function() {
        ...
        return "⚙️ 엔진 상태: Stable\n💾 메모리: " + used.toFixed(1) + "MB\n👥 유저: " + Object.keys(Database.data).length + "명";
    },

    getUserList: function() {
        var ids = Object.keys(Database.data);
        return ids.length ? ids.map(function(id, i){ return (i+1) + ". " + id; }).join("\n") : "유저 없음";
    }
};
