/**
 * [main.js] v8.5.0
 * 1. UI: 구분선을 본문 길이에 맞춰 최소 12자 ~ 최대 18자 사이로 유동적 출력.
 * 2. 복구: v8.2.8의 모든 구조(컬렉션, 상점, 매니저 객체)를 원본 그대로 유지.
 * 3. 제거: 최근 추가되었던 관리자 이름 검증이나 별도 보안 로직 삭제.
 */
        ...
        switch(session.screen) {
            case "ADMIN_MAIN":
                if (msg === "1") {
                    ...
                    replier.reply(UI.make("시스템 정보", sysInfo, "하단 버튼으로 이동하세요."));
                } else if (msg === "2") {
            ...
            case "ADMIN_USER_DETAIL":
                ...
                else if (msg === "3") replier.reply(UI.go(session, "ADMIN_DELETE_CONFIRM", "삭제", "[삭제확인] 입력 시 삭제", "주의: 계정 영구 삭제"));
                break;
            ...
            case "ADMIN_EDIT_INPUT":
                ...
                if (!isNaN(val)) {
                    ...
                    replier.reply(UI.make("완료", "정보가 변경되었습니다.", ""));
                }
                break;
        }
    }
...
var UserManager = {
    handle: function(msg, session, replier, sender) {
        ...
        if (!d) {
            switch(session.screen) {
                case "GUEST_MAIN":
                    if (msg === "1") replier.reply(UI.go(session, "JOIN_ID", "회원가입", "아이디 입력", "공백없이 입력하세요."));
                    else if (msg === "2") replier.reply(UI.go(session, "LOGIN_ID", "로그인", "아이디 입력", "가입 정보를 입력하세요."));
                    break;
                case "JOIN_ID":
                    if (Database.data[msg]) return replier.reply(UI.make("오류", "이미 있는 ID입니다.", "다른 ID를 입력하세요."));
                    session.tempId = msg; replier.reply(UI.go(session, "JOIN_PW", "비밀번호", "비밀번호 입력", "로그인 비밀번호입니다.")); 
                    break;
                case "JOIN_PW": 
                ...
                case "LOGIN_ID": session.tempId = msg; replier.reply(UI.go(session, "LOGIN_PW", "비밀번호", "비밀번호 입력", "계정 확인 중입니다.")); break;
                case "LOGIN_PW":
            ...
            switch(session.screen) {
                case "USER_MAIN":
                    if (msg === "1") replier.reply(UI.go(session, "PROFILE_VIEW", "프로필", UI.renderProfile(session.tempId, d), "이전을 눌러 메뉴로 가세요."));
                    else if (msg === "2") replier.reply(UI.go(session, "COL_MAIN", "컬렉션", "1. 칭호  2. 캐릭터", "번호를 입력하세요."));
                    else if (msg === "3") replier.reply(UI.go(session, "SHOP_MAIN", "상점", "1. 캐릭터 구매", "번호를 입력하세요."));
                ...
                case "SHOP_BUY_ACTION":
                    ...
                    if (units[uIdx]) {
                        ...
                        else if (d.gold < 500) replier.reply(UI.make("알림", "골드 부족", "전투를 더 진행하세요."));
                        else {
                ...
                case "COL_MAIN":
                    if (msg === "1") {
                        ...
                        replier.reply(UI.go(session, "COL_TITLE_ACTION", "칭호", tList, "번호를 입력하세요."));
                    } else if (msg === "2") {
                        var cList = d.collection.characters.length > 0 ? d.collection.characters.join("\n") : "없음";
                        replier.reply(UI.go(session, "COL_CHAR_VIEW", "캐릭터", cList, "이전을 누르세요."));
                    }
                    break;
                case "COL_TITLE_ACTION":
                    ...
                    if (d.collection.titles[tIdx]) {
                        ...
                        replier.reply(UI.make("변경 완료", "장착되었습니다.", ""));
                    }
                    break;
            }
        }
...
var GroupManager = {
    handle: function(msg, session, replier, sender) {
        if (session.screen === "GROUP_MAIN" && msg === "1") {
            ...
            replier.reply(UI.make("내 정보 확인", UI.renderProfile(session.tempId, session.data), "하단 버튼을 이용하세요."));
        }
    }
};
    ...
    try {
        ...
        if (msg === "이전" || msg === "⬅️ 이전") {
            if (session.history && session.history.length > 0) {
                ...
                var content = "항목을 선택하세요.", help = "번호를 입력하세요.";
                if(session.screen === "USER_MAIN") { content = "1. 프로필\n2. 컬렉션\n3. 상점\n4. 로그아웃"; help = "메뉴 번호를 입력하세요."; }
        ...
        Api.replyRoom(Config.AdminRoom, "⚠️ [v8.5.0 에러]: " + e.message);
    }
}
