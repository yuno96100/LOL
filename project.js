/*
 * 🏰 소환사의 협곡 Bot - v20.0 (Ultimate RPG Balance Engine)
 * - [M] Model: 3단 스탠스(공격/파밍/생존) & 3대 타격판정(T/NT/S) & Min-Max 스탯 시스템 도입
 * - [V] View: 거리(Distance) UI 완전 삭제, 중계 시작 시 양측 전략(가위바위보) 공개
 * - [C] Controller: 안전 저장(Sync Save) 및 스케줄러(safeSleep) 기반 무결점 엔진
 */   
...
// ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
var Config = {
    Version: "v20.0 Ultimate Balance Edition",
    AdminRoom: "소환사의협곡관리", 
    BotName: "소환사의 협곡",
...
var Utils = {
    ...
    safeSleep: function(ms) {
        try {
            ...
            for (var i = 0; i < chunks; i++) { java.lang.Thread.sleep(500); }
...
var ContentManager = {
    menus: {
        ...
        profileSub: ["1. 능력치 강화", "2. 스탯 재분배 (포인트 회수)", "3. 능력치 초기화"],
    ...
    screen: {
        ...
        inq: "문의 접수", main: "메인 로비", profile: "내 정보", statSel: "능력치 관리", statCon: "능력치 확인",
    ...
    footer: {
        ...
        selectAction: "작업을 선택하세요.", selectStat: "강화/회수할 스탯 선택", inputPoint: "수치를 입력하세요. (음수 입력 시 포인트 회수)",
    ...
    msg: {
        ...
        notEnoughGold: "골드가 부족합니다.", onlyNumber: "숫자와 마이너스 기호(-)만 입력 가능합니다.",
        ...
        statEnhanceConfirm: function(stat, amt) { return "[" + stat + "] 능력치를 " + amt + "만큼 변경하시겠습니까?"; },
        statEnhanceSuccess: function(stat, amt) { return stat + " 수치가 변경되었습니다."; },
        ...
        battleConnecting: "⚔️ 교전이 시작되었습니다.\n현장 중계를 연결합니다...\n\n(※ 만약 봇이 멈추거나 지연되면 아무 채팅이나 입력해 주세요!)",
    ...
    battle: {
        ...
        director: {
            Aggressive: { MildTrade: "🎙️ 캐스터: 치열한 딜교환이 오갑니다. 양측 모두 물러서지 않네요!", Kiting: "🎙️ 해설: 압도적인 스킬 적중률! 일방적으로 상대를 몰아붙입니다!", Assassinate: "🎙️ 캐스터: 완벽한 락온! 맹렬하게 파고들어 콤보를 꽂아 넣습니다!", Bloodbath: "🎙️ 해설: 엄청난 스킬 난타전!! 피가 쭉쭉 빠집니다!", Countered: "🎙️ 캐스터: 무리한 진입! 오히려 상대의 신들린 무빙에 농락당합니다!", MissAll: "🎙️ 해설: 팽팽한 긴장감 속, 서로의 견제가 엇갈립니다!" },
            Defensive: { NormalFarm: "🎙️ 해설: 안전한 위치에서 훌륭하게 CS를 챙깁니다.", PerfectCS: "🎙️ 캐스터: 엄청난 침착함! 견제 속에서도 흔들림 없이 막타를 다 먹습니다!", CannonMissed: "🎙️ 해설: 아아아!! 사리느라 대포 미니언을 놓쳐버립니다!!", GreedyCS: "🎙️ 캐스터: 파밍하러 잠시 틈을 준 사이 딜교환을 강제당합니다!", ZonedOut: "🎙️ 해설: 라인 장악력이 숨 막힙니다! 디나이 당하며 파밍도 못하고 있어요!", Disaster: "🎙️ 캐스터: 최악의 구도입니다!! 파밍도 놓치고 일방적으로 맞았어요!" },
            baseRecall: "🏠 전장에서 이탈하여 우물로 귀환합니다.",
            shieldPerfect: "🎙️ 해설: 엄청난 방어막 활용!! 맹렬한 공격을 생채기 하나 없이 완벽하게 씹어버립니다!",
            peaceful1: "🏠 상대를 완전히 배제하고 우물로 귀환하여 전열을 가다듬습니다.",
            peaceful2: "🎙️ 해설: 양측 모두 딜교환을 피하고 미니언 파밍에만 집중합니다.",
            peaceful3: "🎙️ 캐스터: 서로 고도의 심리전만 펼칠 뿐, 쉽게 스킬을 빼지 않습니다!"
        },
        effectMap: {
        ...
        logs: {
            ...
            hitTMe: "⏱️[{sec}초] 🎯 거리를 좁혀 [{skill}]을(를) 꽂아 넣습니다! {fxLog}",
            missTMe: "⏱️[{sec}초] 💨 적이 시야를 차단하고 사거리를 주지 않아 락온에 실패했습니다!",
            hitNTMe: "⏱️[{sec}초] ☄️ 날카로운 궤적으로 [{skill}]을(를) 적중시킵니다! {fxLog}",
            missNTMe: "⏱️[{sec}초] 💨 적이 아슬아슬한 무빙으로 투사체를 회피합니다!",
            hitSMe: "⏱️[{sec}초] 💥 적의 동선을 예측하여 [{skill}]을(를) 직격시킵니다! {fxLog}",
            missSMe: "⏱️[{sec}초] 🧠 적이 내 의도를 미리 읽고 장판 밖으로 빠져나갑니다!",
            hitTAi: "⏱️[{sec}초] 🎯 적이 날카롭게 파고들어 [{skill}]을(를) 꽂아 넣습니다! {fxLog}",
            missTAi: "⏱️[{sec}초] 💨 발 빠르게 거리를 벌려 적에게 타겟팅 사거리를 주지 않습니다!",
            hitNTAi: "⏱️[{sec}초] ☄️ 적의 [{skill}]이(가) 정통으로 꽂힙니다! {fxLog}",
            missNTAi: "⏱️[{sec}초] 💨 날아오는 적의 투사체를 가볍게 무빙으로 흘려냅니다!",
            hitSAi: "⏱️[{sec}초] 💥 앗! 내 동선이 읽혔습니다. 적의 [{skill}]에 직격당합니다! {fxLog}",
            missSAi: "⏱️[{sec}초] 🧠 적이 스킬을 쓰는 순간, 의도를 미리 읽고 여유롭게 피합니다!",
            ...
            minionAggro: "🛡️ [미니언 어그로] 무리한 딜교환으로 미니언들에게 두들겨 맞습니다. (-{dmg} HP)",
            towerHitMe: "⏱️[{sec}초] 🔨 라인을 밀어넣고 적 포탑을 거침없이 타격합니다! (포탑 피해 -{dmg})",
            towerHitAi: "⏱️[{sec}초] 🔨 적이 빈 라인을 밀고 우리 포탑을 철거하고 있습니다! (포탑 피해 -{dmg})",
            towerPlate: "💰 [포탑 방패 파괴!] 타워를 채굴하여 150G를 획득했습니다!",
            noAction: "💤 서로 팽팽한 눈치싸움과 견제만 오갑니다.",
            skipMiddle: "... (중략) 치열한 라인전 포지셔닝이 이어집니다!",
            farm: "🌾 [ 파밍 결과 ]\n👤 나: {mCs}개 획득 (+{mGold}G)\n🤖 적: {aCs}개 획득 (+{aGold}G)",
            farmMissed: "❌ 귀환한 사이 포탑에 미니언이 타버립니다.",
            killMe: "\n\n☠️ 치명적인 타격을 입고 전사했습니다! (적 킬 스코어 +1)\n잠시 후 부활하여 라인에 복귀합니다.",
            killAi: "\n\n🔥 완벽한 킬각! 적을 처치했습니다! (내 킬 스코어 +1)\n라인을 장악합니다."
        },
        alerts: {
            ...
            noStrat: { title: "전투 시작 불가", msg: "⚠️ 전략을 먼저 선택하세요! (4~7번 중 하나)" },
...
var BattleDirector = {
    generateLog: function(ctx) {
        ...
        if (realTotalDmg === 0 && ctx.mHits === 0 && ctx.aHits === 0) {
            ...
            if (ctx.strat === 2) return bDir.peaceful2;
...
var BattleView = { 
    Board: {
        render: function(state) {
            ...
            var stratName = ["미선택", "🔥 공격 (딜교환)", "🌾 파밍 (성장)", "🛡️ 생존 (회피)", "🏠 귀환 (우물)"][state.strat || 0];
            content += "[ ⚔️ 2. 전략 카테고리 ]\n▶ 현재작전: " + stratName + "\n";
            content += "4. 공격 (명중↑ / CS하락)\n5. 파밍 (CS집중 / 기본회피)\n6. 생존 (회피극대 / 공격불가)\n7. 귀환 (전장 이탈)\n\n";
    ...
    checkTimeout: function(roomStr, senderStr) {
        ...
        if (s && s.screen !== "IDLE" && (Date.now() - s.lastTime > Config.TIMEOUT_MS)) {
            ...
            if (inBattle) {
                ...
                BotScheduler.schedule(function() {
                    try { Api.replyRoom(roomStr, LayoutManager.renderFrame(ContentManager.title.notice, ContentManager.msg.timeout, false, ContentManager.footer.reStart)); } catch(e){}
                }, 0);
                return true; 
            }
        }
...
function decideAIStrategy(ai, me) {
    ...
    if (myHp < 0.4) return 3;  // 생존(사리기)
    if (myHp > enHp + 0.3 || enHp < 0.3) return 1; // 공격
    if (ai.cs < me.cs - 2) return 1; // 견제
    return (Math.random() > 0.7) ? 3 : 2; // 똑똑한 AI는 파밍 위주에 가끔 사리기
}

function applySpells(actor, target, isAi, logs, bLogs) {
    var didSpell = false;
    var slots = ['d', 'f'];
    ...
    for (var i = 0; i < 2; i++) {
        ...
        if (actor.spells[slot+'Cd'] <= 0) {
            ...
            if (Math.random() * 100 <= reactChance) {
                ...
                else if (spell === "탈진" && actor.status.isAggressive && !target.status.exhaustDur) {
...
var BattleEngine = {
    ...
    calcHit: function(sk, atkSw, defSw, atkHw, defHw, defStatus, atkStrat, defStrat) { 
        if (defStatus.dodgeDur > 0 && (sk == null || sk.t === "AD")) return false; 
        if (defStatus.rootDur > 0 || defStatus.stunDur > 0) return true; 
        if (atkStrat === 3) return false; 
        if (defStrat === 4) return false; 
        
        var type = (sk == null) ? "T" : sk.tt; 
        var hitChance = 0;
        var atkBonus = (atkStrat === 1) ? 15 : 0; 
        var defEvasion = (defStrat === 3) ? 50 : ((defStrat === 1) ? -15 : 0); 

        if (type === "T" || (type === "S" && sk.tt === "S")) {
            hitChance = 100 + (atkSw.acc - defSw.ref) * 0.3 + atkBonus - defEvasion;
        } else if (type === "NT") {
            var finalDefSpd = (defStatus.slowDur > 0) ? defHw.spd * 0.7 : defHw.spd;
            var spdDiff = (atkHw.spd - finalDefSpd) * 0.1;
            hitChance = 80 + (atkSw.acc - defSw.ref) * 0.2 + spdDiff + atkBonus - defEvasion;
        } else {
            hitChance = 80 + (atkSw.int - defSw.com) * 0.3 + atkBonus - defEvasion;
        }
        
        return (Math.random() * 100 <= Math.max(10, Math.min(95, hitChance))); 
    },
    calcCs: function(mySw, enSw, strat, isHit) {
        var base = (strat === 1) ? 20 : (strat === 2 ? 80 : 10);
        var bonus = 0;
        if (strat === 1) bonus += mySw.int * 0.2; 
        if (strat === 2) {
            bonus += mySw.com * 0.2; 
            bonus += (mySw.int - enSw.int) * 0.1; 
        }
        if (strat === 3) bonus += mySw.com * 0.1; 
        if (isHit) bonus -= 15; 
    ...
    playPhase: function(st, stratMe, stratAi, phaseIdx) {
        ...
        for (var sec = 1; sec <= 30; sec++) {
            if (stratMe !== 4) applySpells(me, ai, false, combatLogs, bLogs);
            if (stratAi !== 4) applySpells(ai, me, true, combatLogs, bLogs);
            ...
            if (me.status.stunDur <= 0 && stratMe !== 4) {
                ...
                for (var i=0; i<keys.length; i++) {
                    ...
                    if (skLv > 0 && me.cd[k] <= 0) {
                        ...
                        if (this.evaluateAI(skObj, me, ai, me.status.isAggressive)) {
                            if (me.mp >= skObj.cost) {
                                me.mp -= skObj.cost; me.cd[k] = skObj.cd; usedSkill = true;
                                var type = skObj.tt;
                                var hit = this.calcHit(skObj, me.sw, ai.sw, me.hw, ai.hw, ai.status, stratMe, stratAi);
                                if (hit) {
                                    ...
                                    if (dmg >= ai.hp && flashSlot && aiCanReact) {
                                        ...
                                        var logStr = (type === "T" || type === "S" && skObj.tt === "S") ? bLogs.hitTMe : (type === "NT" ? bLogs.hitNTMe : bLogs.hitSMe);
                                        combatLogs.push(logStr.replace("{sec}", sec).replace("{skill}", skObj.n).replace("{fxLog}", fxLog));
                                        if (aiVulnerable > 0) { combatLogs.push(bLogs.punish); aiVulnerable = 0; }
                                    }
                                } else { 
                                    var missStr = (type === "T" || type === "S" && skObj.tt === "S") ? bLogs.missTMe : (type === "NT" ? bLogs.missNTMe : bLogs.missSMe);
                                    combatLogs.push(missStr.replace("{sec}", sec).replace("{skill}", skObj.n)); 
                                    meVulnerable = 2; 
                                }
                                break; 
                ...
                if (!usedSkill && me.aaTimer >= 1.0) {
                    me.aaTimer -= 1.0; 
                    if (this.calcHit(null, me.sw, ai.sw, me.hw, ai.hw, ai.status, stratMe, stratAi)) {
                        ...
                        if (dmg >= ai.hp && flashSlot && aiCanReact) {
                            ...
                            if(ai.status.invincibleDur > 0) dmg = 0; aRawDmg += dmg;
                            combatLogs.push(bLogs.hitTMe.replace("{sec}", sec).replace("{skill}", "기본 공격").replace("{fxLog}", ""));
                            if (stratAi === 3 && Math.random() < 0.2) combatLogs.push(bLogs.minionAggro.replace("{dmg}", 15 + phaseIdx * 5));
                        }
                    }
                }
            }
            ...
            if (ai.status.stunDur <= 0 && stratAi !== 4) {
                ...
                for (var i=0; i<keys.length; i++) {
                    ...
                    if (skLv > 0 && ai.cd[k] <= 0) {
                        ...
                        if (this.evaluateAI(skObj, ai, me, ai.status.isAggressive)) {
                            if (ai.mp >= skObj.cost) {
                                ai.mp -= skObj.cost; ai.cd[k] = skObj.cd; usedSkill = true;
                                var type = skObj.tt;
                                var hit = this.calcHit(skObj, ai.sw, me.sw, ai.hw, me.hw, me.status, stratAi, stratMe);
                                if (hit) {
                                    ...
                                    if (dmg >= me.hp && flashSlot && meCanReact) {
                                        ...
                                        var logStr = (type === "T" || type === "S" && skObj.tt === "S") ? bLogs.hitTAi : (type === "NT" ? bLogs.hitNTAi : bLogs.hitSAi);
                                        combatLogs.push(logStr.replace("{sec}", sec).replace("{skill}", skObj.n).replace("{fxLog}", fxLog));
                                        if (meVulnerable > 0) meVulnerable = 0;
                                    }
                                } else { 
                                    var missStr = (type === "T" || type === "S" && skObj.tt === "S") ? bLogs.missTAi : (type === "NT" ? bLogs.missNTAi : bLogs.missSAi);
                                    combatLogs.push(missStr.replace("{sec}", sec).replace("{skill}", skObj.n)); 
                                    aiVulnerable = 2;
                                }
                                break;
                ...
                if (!usedSkill && ai.aaTimer >= 1.0) {
                    ai.aaTimer -= 1.0; 
                    if (this.calcHit(null, ai.sw, me.sw, ai.hw, me.hw, me.status, stratAi, stratMe)) {
                        ...
                        if (dmg >= me.hp && flashSlot && meCanReact) {
                            ...
                            if(me.status.invincibleDur > 0) dmg = 0; mRawDmg += dmg;
                            combatLogs.push(bLogs.hitTAi.replace("{sec}", sec).replace("{skill}", "기본 공격").replace("{fxLog}", ""));
                        }
                    }
                }
            }
        ...
        if ((stratMe === 1 || stratMe === 2) && (ai.hp <= 0 || stratAi === 4)) {
        ...
        if ((stratAi === 1 || stratAi === 2) && (me.hp <= 0 || stratMe === 4)) {
        ...
        if(me.status.shield > 0 && mRawDmg > 0) { 
            ...
            mRawDmg -= mBlocked; me.status.shield -= mBlocked; 
        ...
        if(ai.status.shield > 0 && aRawDmg > 0) { 
            ...
            aRawDmg -= aBlocked; ai.status.shield -= aBlocked; 
        ...
        var mGold = 0, mCs = 0, aGold = 0, aCs = 0;
        var csChance = this.calcCs(me.sw, ai.sw, stratMe, aHitCount > 0);
        var aiCsChance = this.calcCs(ai.sw, me.sw, stratAi, mHitCount > 0);
...
var UserController = {
    handle: function(msg, session, senderStr, roomStr) {
        ...
        if (msg === "refresh_screen") {
            ...
            if (session.screen === "STAT_INPUT") return Api.replyRoom(roomStr, LayoutManager.renderFrame(session.temp.statName + " 강화/회수", LayoutManager.templates.inputRequest(null, data.stats[session.temp.statKey], "보유 포인트: " + data.point + " P"), true, f.inputPoint));
        ...
        if (session.screen === "PROFILE_MAIN") {
            if (msg === "1" || msg === "2") { session.screen = "STAT_SELECT"; return UserController.handle("refresh_screen", session, senderStr, roomStr); }
            if (msg === "3") { session.screen = "STAT_RESET_CONFIRM"; return UserController.handle("refresh_screen", session, senderStr, roomStr); }
        }
        ...
        if (session.screen === "STAT_INPUT") {
            ...
            if (isNaN(amt)) return SystemAction.go(roomStr, t.error, m.onlyNumber, function() { UserController.handle("refresh_screen", session, senderStr, roomStr); }); 
            
            var currentStat = data.stats[session.temp.statKey];
            if (currentStat + amt < 0 || currentStat + amt > 100) {
                return SystemAction.go(roomStr, t.error, "스탯은 최소 0, 최대 100까지만 설정 가능합니다.", function() { UserController.handle("refresh_screen", session, senderStr, roomStr); });
            }
            if (amt > 0 && data.point < amt) {
                return SystemAction.go(roomStr, t.fail, "포인트가 부족합니다.", function() { UserController.handle("refresh_screen", session, senderStr, roomStr); });
            }
        ...
        if (session.screen === "STAT_INPUT_CONFIRM") {
            if (msg === "1") {
                ...
                if (currentStat + amt < 0 || currentStat + amt > 100) return SystemAction.go(roomStr, t.error, "스탯 한계치 초과", function() { session.screen = "STAT_SELECT"; UserController.handle("refresh_screen", session, senderStr, roomStr); });
                if (amt > 0 && data.point < amt) return SystemAction.go(roomStr, t.fail, "포인트 부족", function() { session.screen = "STAT_SELECT"; UserController.handle("refresh_screen", session, senderStr, roomStr); });
                
                data.point -= amt; // 음수일 경우 마이너스 마이너스 = 포인트 반환
                data.stats[session.temp.statKey] += amt; Database.save(); 
                
                var actionText = amt > 0 ? " 상승했습니다." : " 감소했습니다. (포인트 반환)";
                return SystemAction.go(roomStr, t.success, session.temp.statName + " 수치가 " + Math.abs(amt) + actionText, function() { session.screen = "STAT_SELECT"; UserController.handle("refresh_screen", session, senderStr, roomStr); });
    ...
    handle: function(msg, session, senderStr, roomStr, userData) {
            ...
            if (msg === "0") {
                ...
                BotScheduler.schedule(function() {
                    ...
                    if (cS && cS.screen === "BATTLE_LOADING") {
                        ...
                        cS.battle.instance = {
                            turn: 1, strat: 0, isBroadcasting: false,
        ...
        if (session.screen === "BATTLE_MAIN") {
            ...
            if (msg === "0" || cleanMsg === "준비완료") {
                ...
                // 🌟 AI의 지능적 판단
                var stratAi = decideAIStrategy(st.ai, st.me); 
                
                var phaseLogs = [];
                var isTurnDeath = false;
                
                var stratNames = ["미선택", "🔥 공격", "🌾 파밍", "🛡️ 생존", "🏠 귀환"];
                var clashLog = "▶ [전략 충돌] 나: " + stratNames[stratMe] + " vs 적: " + stratNames[stratAi] + "\n" + Utils.getFixedDivider() + "\n";

                // 🌟 [1단계] 사전 연산 (거리 개념 완전히 삭제된 엔진 적용)
                for (var i = 1; i <= 3; i++) {
                    var p = bM.playPhase(st, stratMe, stratAi, i);
                    var mentalLog = "";
                    if (st.me.hp <= 0) { 
                        st.ai.kills++; st.me.hp = st.me.hw.hp; st.me.mp = st.me.hw.mp;
                        mentalLog = cB.logs.killMe; isTurnDeath = true; 
                    } else if (st.ai.hp <= 0) { 
                        st.me.kills++; st.ai.hp = st.ai.hw.hp; st.ai.mp = st.ai.hw.mp;
                        mentalLog = cB.logs.killAi; isTurnDeath = true; 
                    }
                    var phaseTitle = cB.screen.phasePrefix + i + cB.screen.phaseSuffix;
                    var phaseContent = (i===1 ? clashLog : "") + p.lckLog + mentalLog + "\n\n" + Utils.getFixedDivider() + "\n[ ⚔️ 타임라인 기록 ]\n" + p.combatLogs + "\n\n" + Utils.getFixedDivider() + "\n" + p.farmLogs + "\n\n" + Utils.getFixedDivider() + "\n[ 📊 수치 변화 ]\n🩸 나: -" + p.aDmg + " HP / 🤖 적: -" + p.mDmg + " HP";
                ...
                // 생존(방어) 선택 시 턴 종료 후 다음 턴 우물 귀환 기본 설정
                if (stratMe === 3) st.strat = 4;
                if (stratAi === 3) st.ai.strat = 4;
                ...
                if (isWin) {
                    ...
                    var aiExpGain = (stratAi === 4) ? 0 : (stratAi === 3 ? 60 : 100); 
                    st.ai.exp += aiExpGain;
                    if (st.ai.exp >= 100) { 
                        ...
                        var stKeys = ["acc", "ref", "com", "int"];
                        st.ai.sw[stKeys[Math.floor(Math.random()*4)]] += 5; 
                    }
                    st.turn++; 
                }
                SessionManager.save();
